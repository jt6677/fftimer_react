...
func (ug userGroup) signin(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	...
	//Authenticate
	usr, err := ug.user.Authenticate(ctx, v.TraceID, nu.Name, nu.Password)
	if err != nil {
		switch err {
		case user.ErrAuthenticationFailure:
			return web.NewRequestError(err, http.StatusUnauthorized)
...
func (ug userGroup) logout(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	session, _ := ug.auth.SessionStore.Get(r, "session")
	//expires session
	...
	return web.Respond(ctx, w, "Logout Successful", http.StatusCreated)
}
